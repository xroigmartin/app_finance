...
- **Estado**: **H0, H1 y H2 hechos**. H2 (Categories) migró el contexto más rico: jerarquía + ámbito como invariantes/servicio, lectura **CQRS** (`CategoryView`/`CategoryQueryPort`) para reproducir el JSON anidado, y la convivencia de `CategoryJpaEntity` con el legado `model/Category` en la misma tabla. ArchUnit con 4 reglas verdes. **378 tests verdes; cobertura global 99,14 %.** Migrados: `accounts`, `categories`. Aún en capas clásicas: transactions, transfers, budgets/recurrencias, categorization, imports, reporting.
- **Próximo paso**: **H3 · Transactions** — agregado `Transaction` con la semántica de **devolución** como invariante (referencia al gasto original por `TransactionId`, no excede el pendiente, solo de gastos, no devolución de devolución; neteo de signo). Mover `TransactionController.applyRefund` al dominio. Buen momento para **mover `TransactionType` a `shared/domain`** (lo comparten transactions/budgets/categorization/imports). Reutilizar la plantilla. **Preguntar antes de arrancar** (ver regla de continuidad arriba).
