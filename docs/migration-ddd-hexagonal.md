...
- [x] **Errores unificados**: retirado el `controller/GlobalExceptionHandler` legado; su mapeo de `DataIntegrityViolationException`→409 vive ahora en `shared/web/DataIntegrityExceptionHandler` (red de última instancia, hermano del `DomainExceptionHandler`). No quedan `ResponseStatusException` legados; todo error de dominio pasa por `DomainException`→advice.
- [x] **Estrangulador retirado por completo**: repuntados los últimos adaptadores que tocaban `repository.*`/`model.*` legado a los repos JPA de cada contexto (las queries de agregación cruzadas se movieron a `TransactionJpaRepository`/`TransferJpaRepository`/`BudgetJpaRepository`; guardas de borrado `AccountUsageAdapter`/`CategoryReferencesAdapter`, lectura `BudgetQueryAdapter`, y los `reporting/.../​*QueryAdapter`). **Eliminados** los 5 `model.*` y los 5 `repository.*` legados y sus tests; `DataSeeder` reescrito sobre los repos/entidades JPA migrados. Los `@DataJpaTest`/Mockito que sembraban con entidades legadas se repuntaron a las entidades JPA (constructores no-arg pasados a `public` para construirlas desde tests cross-package); `Fixtures` recortado y migrado; los tests de query legados (`TransactionRepositoryTest`→`TransactionJpaRepositoryTest`, etc.) reubicados a su contexto.
- [x] **Bug corregido (regresión introducida en el WIP de H9)**: `TransactionJpaRepository.netTotalByAccountUntil` había perdido la lógica por tipo (sumaba todo importe no-devolución en positivo), lo que `ReportingService.balanceUntil` mete directo en el saldo → patrimonio mal calculado. Restaurada la fórmula type-aware (ingreso +, gasto −, devolución invierte); `TransactionJpaRepositoryTest` lo fija (-50/60). Las demás queries migradas eran idénticas al legado; `sumByCategoryAndPeriod`/`netTotal()` no se migraron por ser ya código muerto.
- [x] **ArchUnit estricto global**: quitado `archRule.failOnEmptyShould=false`. **Hallazgo**: con archunit-junit5 1.3.0 las 4 reglas casaban **0 clases** (ASM no parsea bytecode de Java 25 → import vacío) y pasaban vacuamente desde H0b; el `failOnEmptyShould=false` lo enmascaraba. Subido a **1.4.2** (detecta Java 25): las reglas ahora analizan clases reales y pasan con `failOnEmptyShould=true`. Actualizada la sección **Architecture** de `CLAUDE.md`.
- [x] **Suite verde: 483 tests, cobertura global 99,21 % (instrucciones).** Únicos paquetes legados que quedan: `config/` (bootstrap `DataSeeder`), fuera de las capas hexagonales por diseño.
...
- **Estado**: **MIGRACIÓN COMPLETA — H0–H9 hechos.** Los 8 contextos (`accounts`, `categories`, `transactions`, `transfers`, `budgets`, `categorization`, `imports`, `reporting`) están en hexagonal+DDD, más `shared` (kernel + advice). H9 retiró el estrangulador entero: **ya no existen** `model.*` ni `repository.*` legados ni el `GlobalExceptionHandler` legado; `service/`, `controller/`, `dto/`, `model/`, `repository/` están eliminados. El único código fuera de las capas hexagonales es `config/DataSeeder` (bootstrap), por diseño.
- **Calidad**: **483 tests verdes; cobertura global 99,21 % (instrucciones).** ArchUnit (archunit-junit5 **1.4.2**, estricto: `failOnEmptyShould=true`) fija las 4 reglas de frontera y ahora analiza clases reales (con 1.3.0 eran vacuas por el bytecode de Java 25; ver H9).
- **Notas para el futuro**: (a) `config/DataSeeder` podría moverse a un paquete `*/infrastructure/bootstrap` si se quiere meter también bajo las reglas ArchUnit; (b) los constructores no-arg de las `*JpaEntity` se hicieron `public` para construirlas desde tests cross-package (antes `protected`); (c) candidatos VO pendientes: `AccountType` (hoy `String` libre) si se cierra el catálogo de tipos de cuenta.
- **Próximo paso**: ninguno en esta migración. Cualquier dominio nuevo sigue el patrón por contexto descrito en «Estructura de paquetes objetivo» y la convención de nombres por agregado.
