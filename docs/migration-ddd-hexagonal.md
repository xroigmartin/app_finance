...
- **Estado**: **H0 y H1 hechos**. H1 (piloto Accounts) migró el contexto `accounts` end-to-end con el patrón completo (domain puro + puertos + aplicación + adaptadores de persistencia/web + DTOs), validando la plantilla que reutilizarán H2–H8. ArchUnit con 4 reglas verdes. **347 tests verdes; cobertura global 99,04 %.** Conviven la estructura hexagonal (`accounts/`, `shared/`) y la legada (`model`/`controller`/`service`/`repository`); `model/Account` sigue por las referencias `@ManyToOne` de contextos no migrados.
- **Próximo paso**: **H2 · Categories** — el contexto más rico en invariantes (jerarquía de un solo nivel, herencia de tipo/ámbito de subcategoría, reglas de reasignación de ámbito y de recurrencia incompatible, hoy en `CategoryController`). Reutilizar la plantilla de `accounts`. **Preguntar antes de arrancar** (ver regla de continuidad arriba).
