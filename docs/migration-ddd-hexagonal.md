...
- **Estado**: **H0–H4 y H5 completo (H5a + H5b) hechos**. H5b migró las **recurrencias** al contexto `budgets`: agregado `RecurringBudget` (VO `MonthsMask`, historial `RecurrenceAmount` con vigencia, invariante de un importe por mes, `plannedAmount`/`reconcile`), aplicación + lectura CQRS (`RecurringBudgetView`/`RecurringBudgetQueryPort`) + sub-recurso web (`/api/categories/{id}/recurrence`); la reconciliación in situ Hibernate-safe (`uq_amount_vigencia`) vive en el mapper. Se **repuntó** `BudgetQueryAdapter` y `CategoryReferencesAdapter` a los nuevos puertos y se **retiró todo el legado** de recurrencias (`model`/`repository`/`service`/`controller`/`dto` + tests). Estrangulador restante: `model/Budget` + `repository/BudgetRepository` (dashboard H8 + guarda de borrado de categorías). ArchUnit con 4 reglas verdes. **454 tests verdes; cobertura global 99,25 %.** Migrados: `accounts`, `categories`, `transactions`, `transfers`, `budgets` (completo). Aún en capas clásicas: categorization (H6), imports (H7), reporting (H8).
- **Próximo paso**: **H6 · Categorization (reglas + recategorización)** — agregado `CategoryRule` + domain service de matching (patrones con `|`, insensible a mayúsculas/acentos) extraído de `RecategorizationService`; caso de uso "aplicar regla"; persistencia + web + tests; PRD Reglas de categorización. **Preguntar antes de arrancar** (ver regla de continuidad arriba).
