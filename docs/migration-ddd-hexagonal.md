...
- **Estado**: **H0–H6 hechos**. H6 migró la **categorización** al contexto `categorization`: agregado `CategoryRule` (invariante de patrón, `matches`), servicio de dominio `PatternMatcher`, normalización en el shared kernel `TextNormalizer`; aplicación (`CategoryRuleService` + casos de uso) con recategorización de respaldo vía puertos `RuleCategoryCatalog`/`TransactionRecategorizer` (ACL), lectura CQRS (`CategoryRuleView`/`CategoryRuleQueryPort`) y web fino. Se **repuntó** `CategoryReferencesAdapter.hasRule` al nuevo repo y se **retiró** el legado migrado (`RecategorizationService`, `controller/CategoryRuleController` + tests). **Estrangulador restante**: `model/Budget` + `repository/BudgetRepository` (dashboard H8 + guarda de borrado de categorías); y `model/CategoryRule` + `repository/CategoryRuleRepository` vivos **solo** para el `ImportService` legado (H7). ArchUnit con 4 reglas verdes. **478 tests verdes; cobertura global 99,22 %.** Migrados: `accounts`, `categories`, `transactions`, `transfers`, `budgets`, `categorization`. Aún en capas clásicas: imports (H7), reporting (H8).
- **Próximo paso**: **H7 · Imports (orquestación / ACL)** — caso de uso de importación (CSV/Excel) en `imports/application` con `ImportFileParser` como adaptador/ACL; reusar los casos de uso de Transactions/Transfers/Categories y `PatternMatcher`; retirar el legado `model/CategoryRule`+`repository/CategoryRuleRepository` y `service/ImportService`/`ImportFileParser`; tests de parser + caso de uso; PRD Importación. **Preguntar antes de arrancar** (ver regla de continuidad arriba).
