...
- **Estado**: **H0–H7 hechos**. H7 migró la **importación** al contexto `imports`: caso de uso `ImportService` (`ImportTransactions`/`ImportTransfers`) que parsea con `ImportFileParser` (ACL → `ImportFileReader`, devuelve `ImportRow`), resuelve cada fila (cuenta/categoría/regla/respaldo + dedup) y **reutiliza los casos de uso** de los contextos ya migrados a través de puertos de salida propios (`AccountDirectory`/`CategoryDirectory`/`RuleDirectory`/`MovementWriter`/`TransferWriter`) implementados por adaptadores *bridge*; reusa `PatternMatcher`. Endpoints `/import` en los controladores de transactions/transfers ahora delegan en los puertos de entrada. Se **retiró** el legado: `service/ImportService`, `service/ImportFileParser`, `dto/ImportDtos`, `model/CategoryRule`, `repository/CategoryRuleRepository` y sus tests (+ `Fixtures.rule`). **Estrangulador restante**: `model/Budget` + `repository/BudgetRepository` y los demás repos/entidades legados que aún consume el `DashboardService` legado (H8: `service/DashboardService`, `controller/DashboardController`, `dto/DashboardDtos`, `controller/GlobalExceptionHandler`). ArchUnit con 4 reglas verdes. **481 tests verdes; cobertura global 99,22 %.** Migrados: `accounts`, `categories`, `transactions`, `transfers`, `budgets`, `categorization`, `imports`. Aún en capas clásicas: reporting/dashboard (H8) y el cierre transversal (H9).
- **Próximo paso**: **H8 · Reporting (dashboard, solo lectura)** — `reporting`: puertos de consulta (`SummaryQuery`, `MonthlySeriesQuery`, `ByCategoryQuery`, `BudgetStatusQuery`, …) + read models resueltos por adaptadores con las queries de agregación existentes; `DashboardController` de lectura; retirar `service/DashboardService`/`controller/DashboardController`/`dto/DashboardDtos` y los repos legados que ya solo use el dashboard. Tests + PRD Dashboard. **Preguntar antes de arrancar** (ver regla de continuidad arriba).
