...
- [ ] Agregado `RecurringBudget` con `MonthsMask` (máscara de 12 bits con comportamiento) y los importes con vigencia (`validoDesde`) y la **reconciliación in situ** como comportamiento del agregado (hoy en `RecurringBudgetService`). Aplicación + persistencia + sub-recurso web (`/api/categories/{id}/recurrence`). Repuntar `BudgetQueryAdapter` a su propio puerto de recurrencias. Migrar los tests de `RecurringBudgetService`/reconciliación/repo a dominio + adaptador. Retirar el `RecurringBudgetController`/`RecurringBudgetService`/`RecurringBudgetDtos` legados. PRD Presupuestos. **Suite verde.**
...
- **Estado**: **H0–H4 y H5a hechos**. H5a migró el **lado de escritura de presupuestos** (agregado `Budget` con invariantes mes 1..12 e importe positivo) y la **matriz anual como read model CQRS** (`BudgetQueryAdapter`, fusión manual+recurrencia y agregación padre/hijo); se eliminaron `controller/BudgetController`, `service/BudgetService` y `dto/BudgetDtos` legados. Estrangulador: `model/Budget` + `repository/BudgetRepository` siguen vivos (dashboard H8 + guarda de borrado de categorías) y las **recurrencias siguen legadas**, consumidas transitoriamente por el adaptador de lectura. ArchUnit con 4 reglas verdes. **447 tests verdes; cobertura global 99,35 %.** Migrados: `accounts`, `categories`, `transactions`, `transfers`, `budgets` (escritura + matriz). Aún en capas clásicas: recurrencias (H5b), categorization, imports, reporting.
- **Próximo paso**: **H5b · RecurringBudget (escritura)** — migrar el agregado `RecurringBudget` con `MonthsMask` (máscara de 12 bits con comportamiento), importes con vigencia y la **reconciliación in situ** como comportamiento del agregado; aplicación + persistencia + sub-recurso web; repuntar `BudgetQueryAdapter` a su propio puerto de recurrencias; migrar los tests de `RecurringBudgetService`/reconciliación/repo. **Preguntar antes de arrancar** (ver regla de continuidad arriba).
