...
- **Estado**: **H0–H4 hechos**. H4 (Transfers) migró el contexto con **origen ≠ destino** como invariante del agregado y lectura CQRS; se eliminó el `TransferController` legado, pero `model/Transfer` + `repository/TransferRepository` siguen vivos (los usan dashboard y la guarda de borrado de cuentas). ArchUnit con 4 reglas verdes. **427 tests verdes; cobertura global 99,36 %.** Migrados: `accounts`, `categories`, `transactions`, `transfers`. Aún en capas clásicas: budgets/recurrencias, categorization, imports, reporting (estos consumen los repositorios/entidades legados, que conviven en la misma tabla con las nuevas entidades JPA).
- **Próximo paso**: **H5 · Budgets + Recurrencias** — el dominio más rico: agregados `Budget` (por hoja/cuenta/periodo, único) y `RecurringBudget` (`MonthsMask` + importes con vigencia + reconciliación in situ), read model de la matriz anual (CQRS). Persistencia + aplicación + web + tests (los de `BudgetService`/`RecurringBudgetService` pasan a dominio + read model). **Preguntar antes de arrancar** (ver regla de continuidad arriba).
