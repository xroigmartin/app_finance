...
- **`InvestmentQueryAdapter.java` / `InvestmentQueryPort.java`**: esta tarea añade un método nuevo (`closedPositions`); la tarea de API de cotizaciones no toca estos ficheros (opera sobre `PriceProviderPort` y un adaptador nuevo). Confirmado en `docs/plan/precios.md` §2.6: su endpoint de refresco va en `SecurityController`, no en `PortfolioController` — cero solape de fichero con el endpoint de este plan.
...
- **`PortfolioController.java`**: **choque confirmado** con la rama de historial de imports — esta tarea añade `GET .../closed-positions` (§3.4) y esa rama añade `GET .../import-history` en el mismo controller (ya anotado en su propio `docs/plan/historial-imports.md` §7). Hoy el fichero tiene 6 endpoints seguidos (`positions`, `summary`, `valuation-history`, `income`, `performance`, más el `summary` global); es probable que los dos métodos nuevos caigan en zonas distintas y el conflicto de merge sea trivial (métodos distintos, sin solape de lógica), pero revisar al mergear.
