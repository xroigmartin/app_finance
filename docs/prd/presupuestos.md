...
| Migración hexagonal del contexto (H5) | El lado de **escritura** de presupuestos (agregado `Budget`) y la **lectura** de la matriz anual están migrados a arquitectura hexagonal + DDD (paquetes `budgets/{domain,application,infrastructure}`). Las **recurrencias** siguen en capas clásicas (`service/RecurringBudgetService` + `model/RecurringBudget`), que el adaptador de lectura de la matriz consume de forma transitoria. | Completar H5b: migrar el agregado `RecurringBudget` (con `MonthsMask` e importes con vigencia) y repuntar el adaptador de lectura a su propio puerto. |
...
Contexto `budgets` migrado a hexagonal + DDD (escritura + lectura de la matriz; H5a):
...
- Recurrencias (aún en capas clásicas; pendientes de H5b): `model/RecurringBudget.java`, `model/RecurringBudgetAmount.java`, `controller/RecurringBudgetController.java`, `service/RecurringBudgetService.java`, `dto/RecurringBudgetDtos.java`, `repository/RecurringBudgetRepository.java`. La fusión previsto manual+recurrencia vive ahora en `BudgetQueryAdapter` (`annual`, `plannedByCat`), que consume las recurrencias activas vía el repositorio legado de forma transitoria.
...
- Tests de recurrencias (legado): `service/RecurringBudgetServiceTest.java` (upsert con repos mockeados) y, contra Postgres real, `repository/RecurringBudgetReconciliationTest.java` (la reconciliación en sitio que evita violar `uq_amount_vigencia` al editar el importe manteniendo el mes de vigencia; constraints `uq_amount_vigencia`, `chk_recurring_months` y `category_id` único; cascada al borrar el histórico de importes). La constraint `uq_monthly_budgets_account_category_period` se verifica en `repository/ConstraintViolationsTest.java`. Las queries que alimentan la matriz se prueban contra Postgres real en `repository/RecurringBudgetRepositoryTest.java`. El sub-recurso de recurrencias (`/api/categories/{id}/recurrence`) tiene su contrato HTTP en `controller/RecurringBudgetControllerMvcTest.java`.
