...
Tablas `recurring_budgets` y `recurring_budget_amounts` (migración `V5__recurring_budgets.sql`). En el dominio hexagonal son el agregado puro `RecurringBudget` (con el VO `MonthsMask` —máscara de 12 bits— y el historial de `RecurrenceAmount` con vigencia) y sus entidades de persistencia `RecurringBudgetJpaEntity`/`RecurrenceAmountJpaEntity`. Una **recurrencia** declara un pago previsto que se repite (comunidad, hipoteca…) y **solo alimenta el lado "previsto"** de la matriz; **no genera movimientos reales**.
...
| Dos caminos para el "gastado" | La matriz anual usa `sumByExactCategoryAndMonthOfYear` y agrega los padres en el adaptador de lectura `BudgetQueryAdapter`; el dashboard usa `sumByCategoryTreeAndPeriod`. | Ambos son coherentes, pero son dos implementaciones del mismo concepto; valorar unificar. |
| Migración hexagonal del contexto (H5) | Completado (H5a + H5b): presupuestos (agregado `Budget`), la matriz anual (lectura CQRS) y las **recurrencias** (agregado `RecurringBudget` con `MonthsMask` e importes con vigencia) están migrados a arquitectura hexagonal + DDD (paquetes `budgets/{domain,application,infrastructure}`). El adaptador de lectura de la matriz consume las recurrencias por su propio puerto de dominio. | Estrangulador restante: `model/Budget` + `repository/BudgetRepository` siguen vivos para el dashboard (H8) y la guarda de borrado de categorías; se retiran al migrar esos contextos. |
...
Contexto `budgets` migrado a hexagonal + DDD (escritura + lectura de la matriz, H5a; recurrencias, H5b):

- Dominio: `budgets/domain/Budget.java` (agregado: invariantes mes 1..12 e importe positivo; `copyTo`/`isAt`/`reassign`), `BudgetId.java`, puertos `BudgetRepository`, `CategoryCatalog` (hoja + ámbito de cuenta), `AccountExistence`.
- Aplicación: `budgets/application/BudgetService.java` (orden de guardas: duplicado→cuenta→categoría→hoja→ámbito), puertos de entrada `port/{FindBudgets,CreateBudget,UpdateBudget,DeleteBudget,CopyBudgets}`, lectura CQRS `BudgetQueryPort` + read models `BudgetView` / `AnnualBudgetView`.
- Infraestructura: `budgets/infrastructure/persistence/` (`BudgetJpaEntity`, `BudgetJpaRepository`, `BudgetJpaMapper`, `BudgetPersistenceAdapter`, `BudgetQueryAdapter` con el ensamblado de la matriz, `CategoryCatalogAdapter`, `AccountExistenceAdapter`) y `infrastructure/web/` (`BudgetController` fino + `BudgetRequest` / `CopyRequest`).
- Recurrencias (contexto `budgets`, H5b): dominio `budgets/domain/` (`RecurringBudget` —agregado con la reconciliación del historial de importes y el invariante de un importe por mes de vigencia—, VOs `MonthsMask` y `RecurrenceAmount`, `RecurringBudgetId`, puerto `RecurringBudgetRepository`); aplicación `budgets/application/RecurringBudgetService.java` (orden de guardas: 404 inexistente → 400 global/subcategorías/mes/formato), puertos de entrada `port/{FindRecurrence,UpsertRecurrence,DeleteRecurrence}`, lectura CQRS `RecurringBudgetQueryPort` + read model `RecurringBudgetView`; infraestructura `budgets/infrastructure/persistence/` (`RecurringBudgetJpaEntity`, `RecurrenceAmountJpaEntity`, `RecurringBudgetJpaRepository`, `RecurringBudgetJpaMapper` con la **reconciliación en sitio** que respeta `uq_amount_vigencia`, `RecurringBudgetPersistenceAdapter`, `RecurringBudgetQueryAdapter`) y `infrastructure/web/RecurringBudgetController.java` (sub-recurso fino + `RecurringBudgetRequest`). La fusión previsto manual+recurrencia vive en `BudgetQueryAdapter` (`annual`, `plannedByCat`), que consume las recurrencias activas por el puerto de dominio `RecurringBudgetRepository.findActiveByAccount` (agregados, reusando `plannedAmount`).
- Real por categoría y agregación de padres: `repository/TransactionRepository.java` (`sumByExactCategoryAndMonthOfYear`), agregado en `BudgetQueryAdapter` (`leafRow` / `parentRow`).
- Esquema: `db/migration/V1__init.sql`, `V3__categories_per_account.sql`, `V5__recurring_budgets.sql`.
- Tests del contexto migrado: dominio `budgets/domain/BudgetTest.java`; aplicación `budgets/application/BudgetServiceTest.java` (todas las ramas con puertos mockeados); adaptadores contra Postgres real `budgets/infrastructure/persistence/BudgetPersistenceAdapterTest.java` (round-trip + `existsAt` + `findByYearMonth`) y `BudgetQueryAdapterMatrixTest.java` (matriz: previsto manual vs. recurrencia, sumas reales, agregación padre/hijo, ámbito cuenta vs. agregado, split/orden); contrato HTTP `budgets/infrastructure/web/BudgetControllerMvcTest.java`.
- Tests de recurrencias (contexto migrado): dominio `budgets/domain/MonthsMaskTest.java` y `budgets/domain/RecurringBudgetTest.java` (máscara, `plannedAmount`/`amountAt`, reconciliación e invariantes); aplicación `budgets/application/RecurringBudgetServiceTest.java` (todas las ramas con puertos mockeados); contra Postgres real `budgets/infrastructure/persistence/RecurringBudgetPersistenceAdapterTest.java` (round-trip; la **reconciliación en sitio** que evita violar `uq_amount_vigencia` al editar el importe manteniendo el mes de vigencia; scoping activo/cuenta de la query de matriz; cascada al borrar; constraints `uq_amount_vigencia`, `chk_recurring_months` y `category_id` único) y `RecurringBudgetQueryAdapterTest.java` (ensamblado del read model con ids); contrato HTTP `budgets/infrastructure/web/RecurringBudgetControllerMvcTest.java`. La constraint `uq_monthly_budgets_account_category_period` se verifica en `repository/ConstraintViolationsTest.java`.
