...
| Campo | Valor |
|---|---|
| Estado | ✅ **Implementado** — F1, F2, F3 y F3.5 completas (modelo + import Flex + posiciones/valoración multidivisa + rentas + alta manual + rentabilidad TWR/XIRR + historial de imports, con UI) + posiciones cerradas (RF-11); F4 (automatización) **parcialmente implementada**: H4.1 (API externa de cotizaciones, Yahoo Finance) sí, H4.2/H4.3 (Flex Web Service, modo híbrido) no |
| Versión | 0.50 |
...
- Valorar la cartera con las cotizaciones de cierre incluidas en el propio Flex, con refresco bajo demanda contra una API externa de cierre (EOD) desde H4.1 (§6/§10).
...
| RF-11 | El usuario ve el **P&L realizado agregado por año natural e instrumento** (coste promedio, RN-3): una fila por instrumento y año con importe realizado ≠ 0, incluidas las ventas parciales de instrumentos cuya posición sigue abierta hoy — responde "cuánto he ganado/perdido ya con lo que he vendido", sin sustituir el coste FIFO fiscal (backlog §10). |
| RF-12 | El usuario puede consultar el **historial de imports Flex** de una cartera: fecha/hora, fichero, periodo cubierto y el resumen completo (importadas/duplicadas/errores/*warnings*) de cada intento, incluidos los que no importaron ninguna fila nueva. Precursor de la descarga automática (F4, ver `docs/plan/historial-imports.md`). |
...
| `GET` | `/portfolios/{id}/closed-positions` | P&L realizado por instrumento y año natural (RF-11), coste promedio (RN-3); una fila por (instrumento, año) con importe ≠ 0, sin filtros ni paginación. |
...
| `POST` | `/securities/prices/refresh` | Refresco **bajo demanda** (H4.1, sin scheduler) de las cotizaciones de todo el catálogo contra Yahoo Finance (EOD): recorre `Security`, hace upsert de la cotización nueva de cada uno vía `PriceProviderPort` y devuelve `{updated, failed: [{securityId, ticker, reason}]}`. Global, no por cartera — el catálogo de `security` es compartido. Montado en `SecurityController` junto al resto del catálogo (no un controller nuevo). |
| `GET` | `/portfolios/{id}/import-history?page=&size=` | Historial de imports Flex (RF-12), paginado (view CQRS), más reciente primero. Defaults `page=0`/`size=25`. |
...
**Botón Actualizar precios** (H4.1): en la misma barra de herramientas, junto al de import Flex. Sin diálogo — un botón simple con estado de carga ("Actualizando…", deshabilitado mientras corre) que, al terminar, dispara el mismo evento `done` que el import Flex, recargando posiciones/valoración/rentabilidad en la página activa; solo muestra mensaje junto al botón si la petición falla (error de red/API). El resultado del refresco (qué se actualizó) no se resume junto al botón: se ve directamente en la fila de cada posición — precio (§7, tabla de posiciones) y el badge "a coste" (RN-6) para las que no tienen cotización — así que no hay resumen agregado que mantener sincronizado con la tabla. **Revisado (2026-07-27)**: se eliminó el resumen inline ("N instrumentos actualizados"/"M sin cotización") de la barra por redundar con la tabla y por legibilidad; la fecha de cada cotización pasó de texto fijo bajo el precio a un botón-tooltip (icono ⓘ, `title` nativo "Última cotización: dd/MM/yyyy") junto al precio, para no ensuciar la celda cuando no hace falta consultarla.
...
- **Instrumento cotizado en varias divisas / mercados (cross-listing)**: la identidad de `security` es **ISIN + divisa de cotización** (única por par). Un valor cross-listed (mismo ISIN en dos mercados) aparecería como **dos securities** y la posición se mostraría partida en dos filas; no ocurre en la cartera actual (ningún ISIN repetido en dos divisas). Caso límite conocido, sin maquinaria de consolidación en v1. Matiz importante: la **divisa no distingue mercados de la misma moneda** (Euronext París y Ámsterdam cotizan ambos en EUR) — para eso está el `exchange` (`listingExchange` del Flex) y el `figi` (único por cotización), que se capturan como metadatos del `security` desde v1. **Resuelto parcialmente (H4.1, 2026-07-26)**: el `exchange` (no el `figi`) es ya la llave de consulta real del precio — `YahooExchangeResolver` lo traduce al sufijo de símbolo que espera el endpoint de Yahoo Finance, cubriendo solo los mercados hoy presentes en la cartera (LSE→`.L`, NASDAQ→*sin sufijo*, SBF→`.PA`, AEB→`.AS`, BVME→`.MI`, IBIS→`.DE`; tabla verificada con llamadas reales el 2026-07-26, incluida Milán — descartada en la evaluación previa de Alpha Vantage por no cubrirla, sí cubierta por Yahoo); un `exchange` no mapeado o vacío simplemente no se refresca (sin excepción). La revisión de si la identidad debe pasar a ISIN + exchange sigue **pendiente** (no forzada: el `exchange`/sufijo actual resuelve el símbolo sin tocar el modelo de identidad, tal como preveía este mismo párrafo).
...
- **Scheduler/refresco automático de precios** (H4.3, "modo híbrido"): el botón bajo demanda (H4.1, ✅ implementado, ver §6) cubre el refresco manual; falta el job automático (idea de especificación pendiente de validar: ~10 min tras el cierre de mercado, requiere horario por mercado — la cartera mezcla LSE/NASDAQ/SBF/IBIS/BVME/AEB).
- Benchmarks (comparar TWR contra un índice) — depende de tener precios frescos (H4.1), ahora desbloqueado.
...
| Serie de valoración dispersa | **Se refina progresivamente (2026-07-26, H4.1)**: cada pulsación de "Actualizar precios" añade una cotización EOD más a la serie (además de las de import), así que TWR/XIRR ganan densidad con el uso. **Sigue sin haber backfill histórico**: el refresco solo trae el último cierre disponible, no rellena huecos pasados — eso seguiría requiriendo un import Flex o un adaptador de histórico aparte. | Evaluar backfill histórico si la dispersión pasada sigue siendo un problema real tras un tiempo de uso del botón. |
...
| ~~Literal de divisa en peniques~~ | **Resuelto (2026-07-26).** `YahooFinancePriceProvider` reconoce el literal `"GBp"` (minúscula, confirmado con una llamada real al endpoint contra `ZEG.L` el 2026-07-26 — no era `"GBX"`) como peniques de LSE (§2.3 de `docs/plan/precios.md`); cualquier otra divisa de respuesta que no case con `Security.currency` se descarta y se loguea WARN (`business.investments`), para detectar mercados nuevos con el mismo problema si Yahoo cambiara el literal. | — |
| Endpoint no oficial (Yahoo Finance) | El adaptador usa `query1.finance.yahoo.com/v8/finance/chart/{symbol}`, sin SLA ni contrato — puede cambiar de formato o bloquearse sin aviso (mitigado el riesgo de cookie+crumb: verificado el 2026-07-26 que no hace falta, solo un `User-Agent` de navegador; ver revisión en `docs/plan/precios.md`). El aislamiento por `PriceProviderPort` permite sustituirlo sin rediseñar el resto si deja de funcionar. | Ninguna acción por ahora; vigilar si el botón de refresco empieza a fallar en bloque (no instrumento a instrumento) como señal de cambio de formato. |
| Motivos de fallo del refresco de precios | `PriceRefreshService` solo distingue **"Sin mercado configurado"** (detectable sin acoplarse al proveedor) de **"Sin cotización disponible"** (genérico): `PriceProviderPort.latestQuotes` colapsa símbolo no encontrado/timeout/respuesta malformada en una lista vacía sin motivo, así que el servicio no puede distinguirlos sin acoplar la capa de aplicación a Yahoo Finance. Distinguible solo en el log WARN del adaptador. | Si el resumen del botón necesita más granularidad, ampliar el contrato de `PriceProviderPort` para que devuelva un motivo, no solo una lista. |
...
- **Infraestructura** (`infrastructure/web/`): `InvestmentTransactionController` (H2.3: `GET/POST /portfolios/{id}/transactions` con filtros `type/from/to/securityId` + `PUT/DELETE /transactions/{id}`, DTO `InvestmentTransactionRequest` con validación estructural mínima); `PortfolioController` (CRUD `/api/investments/portfolios`, `POST .../{id}/import` multipart —campo `file`; límite subido a 10 MB en `application.properties`, los Flex anuales rondan 1,5 MB—, `GET .../positions|summary|valuation-history|income|performance|closed-positions`, `GET /api/investments/summary`) y `SecurityController` (CRUD `/api/investments/securities` + `POST /securities/prices/refresh`, H4.1) con DTOs `PortfolioRequest`/`PortfolioResponse`/`SecurityRequest`/`SecurityResponse`/`PriceRefreshResponse`; las views CQRS se serializan tal cual. Errores vía `shared.web.DomainExceptionHandler` (400/404/409 `problem+json`).
...
- **Infraestructura** (`infrastructure/prices/`, H4.1): `YahooFinancePriceProvider implements PriceProviderPort` contra el endpoint no oficial `GET /v8/finance/chart/{symbol}` de Yahoo Finance (símbolo vía `YahooExchangeResolver`, rango `5d`/intervalo `1d`); `YahooChartResponse` (DTO Jackson anidado, tolerante a campos desconocidos) — toma el último cierre no nulo de la serie diaria (nunca `regularMarketPrice`, que puede ser una cotización en vivo) y su fecha, calculada desplazando el timestamp UTC por el `gmtoffset` del mercado antes de truncar a día; conversión GBp/peniques→GBP (§2.3 de `docs/plan/precios.md`, literal `"GBp"` verificado empíricamente el 2026-07-26, ver §11); cualquier fallo (símbolo no encontrado → HTTP 404, timeout, respuesta malformada, divisa inesperada) → lista vacía + WARN en `business.investments`, nunca excepción hacia el puerto. `PriceProviderConfig` (`@Configuration`) construye el `RestClient` (timeout 5 s, `User-Agent` de navegador — el endpoint no exige cookie+crumb, solo rechaza clientes que no parecen un navegador — vía `spring-boot-restclient`) y expone el bean `PriceProviderPort`; sin API key que gestionar.
- **Historial de imports** (H-imp.1–H-imp.4, RF-12): dominio `ImportRecord` (record + factory `of()`, valida `portfolioId`/`importedAt`/`toDate` no nulos) y VO `ImportRowIssue` (`domain/`, duplica la forma de `FlexRowError` para no depender hacia arriba de `application/`) + puerto `ImportRecordRepository` (solo `save`); `FlexImportService.importReport` lo persiste al final de cada intento, dentro de la misma transacción, incluidos los imports sin filas nuevas; migración `V8__import_record.sql` (`errors`/`warnings` como `jsonb`, sin tabla hija) con `ImportRecordJpaEntity`/`ImportRecordJpaMapper` (serializa con el `ObjectMapper` de Jackson 3 — `tools.jackson.databind`, el motor real del proyecto vía `spring-boot-starter-jackson`, no el clásico `com.fasterxml.jackson.databind`) y `ImportRecordPersistenceAdapter`; lectura CQRS `ImportRecordQueryPort`/`ImportRecordView` (reutiliza `FlexRowError` porque esta vista sí vive en `application/`) + `ImportRecordQueryAdapter` (lee `ImportRecordJpaRepository` directamente, sin pasar por el agregado de dominio — es un listado, no un cálculo) y endpoint `GET /portfolios/{id}/import-history` en `PortfolioController`.
...
- **Frontend** (H1.11–H1.13, H2.3–H2.4, H3.4): rentabilidad — tarjetas KPI «Rentabilidad (TWR)» (acumulada del periodo) y «Rentabilidad (XIRR)» (anualizada) con «—» si no calculable en `pages/investments-dashboard/`, columnas TWR/XIRR por posición en la tabla (con tooltip ⓘ explicando cada métrica en lenguaje llano) y gráfico de barras horizontales «Rentabilidad por posición (%)» (XIRR y TWR por instrumento, ordenado por XIRR) junto al de P&L, ambos en `pages/investments-dashboard/`; modelo `InvestmentPerformance`/`PositionPerformance` y `getInvestmentPerformance()` en `api.service.ts`. Resto del frontend: tarjeta de **patrimonio invertido** en el dashboard doméstico (RF-10: agregado en EUR + fecha de valoración + desglose con >1 cartera; oculta sin carteras, "—" ante error de API — ver PRD Dashboard RF-8); tres páginas lazy bajo Inversión en el menú lateral (H1.13, §7): `pages/investments-dashboard/` (ruta `/investments/dashboard`, "Panel general") con la cabecera de KPIs (valor total con fecha de valoración o aviso "a coste", aportado neto, P&L latente € y %, efectivo por divisa, dividendos del año en bruto) y los gráficos Chart.js integrados con `ThemeService`: **Evolución: valor vs aportado** a ancho completo en su propia fila (aportado escalonado), y debajo una fila a 3 columnas con el donut de **Asignación de la cartera** (efectivo como porción más), las barras horizontales divergentes de **P&L latente por posición** y las de **Rentabilidad por posición (%)** — estas dos últimas con un botón-tooltip ⓘ en el título explicando qué compara el gráfico, en lenguaje llano; `pages/investments-positions/` (ruta `/investments/positions`, "Posiciones", nueva página): tabla de posiciones de la cartera seleccionada (cantidades negativas en rojo RN-4; badge "a coste" sin cotización RN-6; precio junto a un botón-tooltip ⓘ con la fecha de la última cotización en su `title`; cabeceras TWR/XIRR con su propio botón-tooltip ⓘ explicando qué mide cada métrica, en lenguaje llano — clase compartida `.info-btn`/`.th-info`), carga sus propias posiciones/rentabilidad vía `getPositions()`/`getInvestmentPerformance()` (no depende del Panel general); y `pages/investments-operations/` (ruta `/investments/operations`, "Operaciones") con las pestañas **Operaciones** — listado filtrable por tipo/fechas/instrumento (etiquetas de tipo compartidas `INVESTMENT_TYPE_LABELS`), columna de origen (manual/importado por `externalId`), editar (reutiliza `investment-transaction-dialog.edit(tx)` vía `InvestmentToolbar.edit(tx)`) y borrar con confirmación; **Dividendos** — gráfico de barras mensuales apiladas por instrumento (bruto, con el neto en el tooltip) con selector de año y vista «Todo (por año)», tabla de cobros por instrumento (bruto/retenido/neto + total) y totales de comisiones/retenciones pagadas del periodo; los intereses sin instrumento agrupan como «Intereses»; **Cerradas** (H-CP.4, RF-11) — mismo patrón que Dividendos: `loadClosedPositions()` se dispara junto al resto de cargas al cambiar de cartera; `closedRows` (getter) agrega `ClosedPosition[]` por instrumento dentro del año seleccionado (o de todos con "Todo (por año)") y `closedTotal` suma la columna; sin gráfico, solo tabla + fila TOTAL con color por signo; e **Importaciones** (H-imp.6, RF-12) — última pestaña: tabla paginada (`app-pagination`) y fila expandible de detalle (`toggleHistoryDetail`, reutiliza el markup `<ul class="errors">`/`<ul class="warnings">` de `flex-import-dialog.ts`, duplicado en `investments-operations.scss` en vez de extraído a componente compartido — solo 2 usos), carga perezosa (`loadImportHistory()` solo al activar la pestaña o cuando ya está activa, `reloadImportHistoryIfActive()`). Las cuatro páginas comparten `InvestmentContextService` (`src/app/investment-context.service.ts`: carteras, instrumentos, cartera seleccionada vía `portfolioId$`, alta inline de cartera) y `components/investment-toolbar.ts` (selector/alta de cartera + diálogo `components/flex-import-dialog.ts` + botón "Actualizar precios" (H4.1) con `refreshPrices()`/`refreshingPrices`/`priceRefreshResult`/`priceRefreshError`; `FlexImportDialog.done` emite siempre, también con `imported === 0`, para que un reimport "todo duplicado" refresque Operaciones/Dividendos/Importaciones igual). Modelos en `models.ts` (`Portfolio`, `PositionView`, `PortfolioSummary`, `ValuationPoint`, `InvestmentsSummary`, `FlexImportResult`, `InvestmentTransactionView`, `InvestmentIncome`, `ClosedPosition`, `PriceRefreshResult`, `PriceRefreshFailure`, `ImportRecordView`…) y llamadas en `api.service.ts` (incluye `refreshPrices()`, `getImportHistory()`).
- **Frontend — enlace desde el diálogo al historial** (H-imp.7, RF-12): `FlexImportDialog.goToImportHistory()` (`flex-import-dialog.ts`) cierra el diálogo y navega con `Router.navigate(['/investments/operations'], { queryParams: { tab: 'importaciones' } })`; botón visible solo si `result.errors.length > 0 || result.warnings.length > 0`. `InvestmentsOperationsPage.ngOnInit` (`investments-operations.ts`) se suscribe a `ActivatedRoute.queryParamMap`: si `tab === 'importaciones'` activa esa pestaña (`setTab`) y limpia el query param con `router.navigate([], { relativeTo: route, queryParams: {}, replaceUrl: true })` para que no quede fijado en recargas posteriores. Funciona igual llamado desde el Panel general o desde la propia Operaciones porque la cartera es estado compartido (`InvestmentContextService`), no viaja por la URL.

Pendiente: H4.2 (Flex Web Service) y H4.3 (refresco automático/programado) en backlog.
