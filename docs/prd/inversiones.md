...
**Botón Actualizar precios** (H4.1): en la misma barra de herramientas, junto al de import Flex. Sin diálogo — un botón simple con estado de carga ("Actualizando…", deshabilitado mientras corre) que, al terminar, muestra un resumen inline ("N instrumentos actualizados" y, si aplica, "M sin cotización disponible") o un error si la petición falla. Tras un refresco con éxito dispara el mismo evento `done` que el import Flex, recargando posiciones/valoración/rentabilidad en la página activa.
