...
| Campo | Valor |
|---|---|
| Estado | ✅ **Implementado** — F1, F2 y F3 completas (modelo + import Flex + posiciones/valoración multidivisa + rentas + alta manual + rentabilidad TWR/XIRR, con UI); F4 (automatización) **parcialmente implementada**: H4.1 (API externa de cotizaciones, Twelve Data) sí, H4.2/H4.3 (Flex Web Service, modo híbrido) no |
| Versión | 0.46 |
...
| Literal de divisa en peniques (Twelve Data) | `TwelveDataPriceProvider` reconoce `"GBX"`/`"GBp"` como peniques de LSE (§2.3 de `docs/plan/precios.md`) **sin verificación empírica contra la API real** (no había `FINANCE_TWELVEDATA_API_KEY` disponible al implementarlo); cualquier otra divisa de respuesta que no case con `Security.currency` se descarta y se loguea WARN (`business.investments`). | Verificar contra una llamada real a `/eod` con un ticker LSE (p. ej. ZEG) en cuanto haya API key configurada; ajustar el literal si difiere. |
| Motivos de fallo del refresco de precios | `PriceRefreshService` solo distingue **"Sin mercado configurado"** (detectable sin acoplarse al proveedor) de **"Sin cotización disponible"** (genérico): `PriceProviderPort.latestQuotes` colapsa símbolo no encontrado/timeout/cuota agotada en una lista vacía sin motivo, así que el servicio no puede distinguirlos sin acoplar la capa de aplicación a Twelve Data. Distinguible solo en el log WARN del adaptador. | Si el resumen del botón necesita más granularidad (p. ej. avisar específicamente de cuota agotada), ampliar el contrato de `PriceProviderPort` para que devuelva un motivo, no solo una lista. |
...
| Hito | Contenido | Tests |
|---|---|---|
| H4.1 | ✅ **Implementado (2026-07-26).** Adaptador `TwelveDataPriceProvider implements PriceProviderPort` contra el endpoint EOD de Twelve Data (free tier, 800 peticiones/día): `TwelveDataExchangeResolver` (dominio) traduce `exchange` IBKR → `mic_code`; conversión GBX/peniques→GBP para LSE (§9); tolerante (símbolo no encontrado/timeout/cuota agotada → lista vacía + WARN, nunca excepción). Caso de uso `PriceRefreshService implements RefreshPrices`: recorre el catálogo, upserta cotizaciones nuevas, agrega `{updated, failed}` sin que un fallo puntual aborte el resto. Endpoint `POST /api/investments/securities/prices/refresh` en `SecurityController` + botón "Actualizar precios" en `investment-toolbar.ts` (sin diálogo, resumen inline). Sin scheduler (bajo demanda únicamente) y sin migración (`price_quote` ya soportaba el upsert). | Unitarios de dominio (`TwelveDataExchangeResolverTest`), del adaptador contra `MockRestServiceServer` (`TwelveDataPriceProviderTest`), aplicación con puertos mockeados (`PriceRefreshServiceTest`), `@WebMvcTest` (`SecurityControllerMvcTest`) y Vitest del toolbar. |
...
- **Infraestructura** (`infrastructure/prices/`, H4.1): `TwelveDataPriceProvider implements PriceProviderPort` contra el endpoint `GET /eod` de Twelve Data (símbolo + `mic_code` vía `TwelveDataExchangeResolver` + `apikey`); `TwelveDataEodResponse` (DTO Jackson tolerante a campos desconocidos y a la forma de error `status="error"`); conversión GBX/peniques→GBP (§2.3 de `docs/plan/precios.md`, literales `"GBX"`/`"GBp"` sin verificar empíricamente, ver deuda técnica §11); cualquier fallo (símbolo no encontrado, timeout, cuota agotada, divisa inesperada) → lista vacía + WARN en `business.investments`, nunca excepción hacia el puerto. `PriceProviderConfig` (`@Configuration`) construye el `RestClient` (timeout 5 s, vía `spring-boot-restclient` — no la trae `spring-boot-starter-web` transitivamente en el Boot 4 modularizado) y expone el bean `PriceProviderPort`; clave `finance.twelvedata.api-key` ← `FINANCE_TWELVEDATA_API_KEY` (vacía por defecto).
