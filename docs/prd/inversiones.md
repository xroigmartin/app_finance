...
| Campo | Valor |
|---|---|
| Estado | 🚧 En implementación — F1 en curso (H1.1–H1.9 completados) |
| Versión | 0.30 |
...
Implementado hasta H1.9 (`backend/src/main/java/com/xroig/finance/investments/`):
...
Pendiente (estructura prevista): caso de uso `ImportFlexReport` con endpoint `POST /portfolios/{id}/import` — H1.10; views `IncomeView` — F2 y `PerformanceView` + `PerformanceCalculator` — F3; frontend `pages/investments/`, modelos en `models.ts`, llamadas en `api.service.ts` — H1.11.
