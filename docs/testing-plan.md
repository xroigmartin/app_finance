...
- [ ] **M4..Mn**: resto de controllers (`TransferController`, `CategoryRuleController`, `DashboardController`, `RecurringBudgetController`).
...
- **Estado**: **Nivel 2 cerrado** (T0–T4); **M0** (infra), **M1** (`TransactionController`), **M2** (`CategoryController`) y **M3** (`BudgetController`) hechos. 266 tests verdes en la suite completa. Patrón Nivel 3 fijado: `@WebMvcTest` + `@MockitoBean` + `MockMvcTester`.
- **Próximo paso**: **M4** — `TransferController` y `CategoryRuleController` (CRUD más pequeño) por el contrato HTTP, y cerrar con `DashboardController` y `RecurringBudgetController`. Es continuación de la misma etapa (Nivel 3 ya arrancado).
