...
- [x] **Bootstrap · `DataSeederTest`** (cierra el hueco que E4 aplazó): un `@SpringBootTest` arranca el **contexto completo** contra Postgres real para que el `CommandLineRunner` del `DataSeeder` corra de verdad (lo que ni Mockito ni los slices `@WebMvcTest`/`@DataJpaTest` ejercitan). Con `finance.seed-demo=true` cubre la rama demo (cuentas + categoría "Luz" por cuenta + 12 meses de movimientos) y re-invocar el runner fija la idempotencia (`count()==0`). **Usa contenedor propio, no el singleton compartido**: el runner *commitea* la siembra al arrancar y contaminaría los tests de repositorio aislados por rollback. Subió `DataSeeder` de 0 % a 99,8 % y el global de 91,4 % a 99,0 %. 4 tests verdes.
