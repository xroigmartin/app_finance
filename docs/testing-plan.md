...
- [ ] **M3..Mn**: resto de controllers (`BudgetController`, `TransferController`, `CategoryRuleController`, `DashboardController`, `RecurringBudgetController`).
...
- **Estado**: **Nivel 2 cerrado** (T0–T4); **M0** (infra), **M1** (`TransactionController`) y **M2** (`CategoryController`) hechos. 247 tests verdes en la suite completa. Patrón Nivel 3 fijado: `@WebMvcTest` + `@MockitoBean` + `MockMvcTester`.
- **Próximo paso**: **M3** — `BudgetController` (matriz anual, hoja/duplicados, copy) por el contrato HTTP, y seguir con el resto. Es continuación de la misma etapa (Nivel 3 ya arrancado).
