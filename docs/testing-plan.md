...
- [x] **M · Puerta 90 %** global de la aplicación: **JaCoCo global 91,4 %** (5459/5971), 307 tests verdes. Todos los controllers, services y la capa de repositorio quedan al 93–100 %. **Lo único a 0 % sigue siendo `config/DataSeeder` (454 instr.)** más unas ramas de `FinanceApplication`/bootstrap: código de arranque no ejercitable por slices ni Mockito. La puerta del 90 % se cumple a pesar de ello; cerrar ese hueco requiere un `@SpringBootTest` de contexto+siembra (ver `BootstrapContextTest` más abajo si se aborda).

---

## Estado actual / Próximo paso

- **Estado**: **Nivel 3 completo** (M0–M5) y **puerta M · 90 % cumplida** (JaCoCo global **91,4 %**, 307 tests verdes). Los tres niveles del plan están cerrados; todos los controllers tienen su test de contrato HTTP `@WebMvcTest`.
- **Próximo paso (opcional)**: cerrar el único hueco grande de cobertura, `config/DataSeeder` (0 %), con un `@SpringBootTest` que arranque el contexto completo contra Postgres real y verifique la siembra (categorías por defecto siempre; cuentas + 12 meses con `finance.seed-demo=true`). No es necesario para la puerta del 90 %, pero es lo que E4 dejó aplazado.
