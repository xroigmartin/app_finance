...
- [ ] **M5..Mn**: `DashboardController` y `RecurringBudgetController`.
...
- **Estado**: **Nivel 2 cerrado** (T0–T4); **M0** (infra), **M1** (`TransactionController`), **M2** (`CategoryController`), **M3** (`BudgetController`) y **M4** (`TransferController` + `CategoryRuleController`) hechos. 288 tests verdes en la suite completa. Patrón Nivel 3 fijado: `@WebMvcTest` + `@MockitoBean` + `MockMvcTester`.
- **Próximo paso**: **M5** — `DashboardController` (endpoints de agregación, solo lectura) y `RecurringBudgetController` (sub-recurso `/api/categories/{id}/recurrence`), para cubrir los dos controllers que faltan y llegar a la puerta `M · 90 %`. Es continuación de la misma etapa (Nivel 3 ya arrancado).
